...
/** Phase C2: Semantic colors — Safe #10B981, Caution/Avoid #b42318 */
...
/** Phase C2: Minimalist 4px confidence bar using semantic colors */
function ConfidenceBar({
  ...
  score: number;
  level: string;
  riskLabel: CheckResult['riskLabel'];
}) {
  const fillColor =
    riskLabel === 'Safe'
      ? '#10B981'
    ...
    <div className="mt-3">
      ...
      <div className="h-1 rounded-full bg-gray-200 overflow-hidden">
        <div
          ...
          style={{ width: `${Math.min(100, Math.max(0, score))}%`, backgroundColor: fillColor }}
        />
      </div>
    </div>
  );
...
export default function ResultPage() {
  ...
  const heroClass =
    result.riskLabel === 'Safe'
      ? 'aa-hero-safe'
      : result.riskLabel === 'Caution'
        ? 'aa-hero-caution'
        : result.riskLabel === 'Avoid'
          ? 'aa-hero-avoid'
          : 'aa-hero-neutral';
  ...
  return (
    <div className="px-4 py-6 max-w-xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Allergy Angel</h1>
          <p className="text-sm font-normal text-gray-600 mt-1">{result.summary}</p>
        </div>
        <div className={`rounded-2xl p-6 min-w-[200px] ${heroClass}`}>
      ...
      {result.detectedEntities.length > 0 && (
        <div className="mt-6 aa-soft-card p-6">
          <h2 className="text-base font-semibold text-gray-900">Detected</h2>
      ...
      {result.reasons.length > 0 && (
        <div className="mt-6 aa-soft-card p-6">
          <h2 className="text-base font-semibold text-gray-900">Why</h2>
      ...
      {result.missingInfo.length > 0 && (
        <div className="mt-6 aa-soft-card p-6">
          <h2 className="text-base font-semibold text-gray-900">To be more confident</h2>
          <ul className="mt-4 list-disc list-inside text-sm text-gray-700 space-y-2">
      ...
      {/* Next Steps */}
      <div className="mt-6 aa-soft-card p-6">
        <h2 className="text-base font-semibold text-gray-900">What next</h2>
      ...
      {canShowSuggestion && suggestion && (
        <div className="mt-6 aa-soft-card p-6 bg-gray-50/50">
          ...
          <div className="mt-4 flex flex-col sm:flex-row gap-4">
            <button
              ...
              className="min-h-[44px] w-full sm:w-auto rounded-2xl px-4 py-3 text-sm font-semibold bg-emerald-600 text-white hover:bg-emerald-700"
            >
              Add to profile
            </button>
            ...
            <button
              ...
              className="min-h-[44px] w-full sm:w-auto rounded-2xl px-4 py-3 text-sm font-semibold border border-slate-200 text-gray-900 hover:bg-slate-50 bg-transparent"
            >
              Dismiss
            </button>
          </div>
      ...
      {/* Actions — Phase C2: primary + ghost secondary, min 44px for thumb */}
